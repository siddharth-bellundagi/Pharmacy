...
const ReportsAnalytics: React.FC = () => {
  ...
  const periodRevenue = filteredSales.reduce((sum, sale) => sum + sale.total, 0);
  const periodTransactions = filteredSales.length;
  const averageTransaction = periodTransactions > 0 ? periodRevenue / periodTransactions : 0;
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-2xl font-semibold text-gray-900">${periodRevenue.toFixed(2)}</p>
              <p className="text-xs text-gray-500">{periodTransactions} transactions</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-2xl font-semibold text-gray-900">${averageTransaction.toFixed(2)}</p>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-2xl font-semibold text-gray-900">{todaySales.length}</p>
              <p className="text-xs text-gray-500">
                ${todaySales.reduce((sum, sale) => sum + sale.total, 0).toFixed(2)}
              </p>
            </div>
          </div>
        </div>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div className="flex-1 bg-gray-200 rounded-full h-2">
                    <div 
                      ...
                      style={{ 
                        width: `${Math.max((day.revenue / Math.max(...dailySales.map(d => d.revenue))) * 100, 2)}%` 
                      }}
                    />
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-semibold text-gray-900">{day.sales}</p>
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="flex-1 bg-gray-200 rounded-full h-2 w-24">
                        <div 
                          ...
                          style={{ 
                            width: `${Math.max((revenue / Math.max(...Object.values(salesByCategory))) * 100, 2)}%` 
                          }}
                        />
                      </div>
                    </div>
                    <span className="text-sm font-semibold text-gray-900">
                      PKR ${revenue.toFixed(2)}
                    </span>
                  </div>
                ))
            ) : (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
              ...
              <span className="font-semibold text-gray-900">PKR ${totalRevenue.toFixed(2)}</span>
            </div>
            <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
              <span className="text-sm text-gray-600">Total Sales</span>
              <span className="font-semibold text-gray-900">{sales.length}</span>
            </div>
            <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
